...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        # 判断图像是否能被块平分   assert (判定式), '提示句' 如果不满足判定式 则报提示语句
...
class Attention(nn.Module):
    def __init__(self,
        ...
        '''
        num_heads = 12
        head_dim = 768 // 12 = 64 （Attention is all you need论文中提到的dk=dv=dim/h）
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape
        # qkv进行注意力操作，reshape进行multi-head的维度分配，permute维度调换以便后续操作
        # qkv(): -> [batch_size, num_patches + 1, 3 * total_embed_dim] 如 [b,197,3*768]
        # reshape: -> [batch_size, num_patches + 1, 3, num_heads, embed_dim_per_head] 如 [b,197,3,12,64]
        # permute: -> [3, batch_size, num_heads, num_patches + 1, embed_dim_per_head] 如 [3,b,12,197,64]
        ...
        attn = attn.softmax(dim=-1) # 针对每一行进行softmax操作处理
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = nn.Linear(in_features, hidden_features)  # 隐藏层通道数升维为原来4倍。
        self.act = act_layer()  # GELU是更加平滑的relu
...
class Block(nn.Module):
    ...
    def __init__(self,
                 dim,
                 num_heads,
                 mlp_ratio=4.,  # MLP 隐藏层和 embed_dim 的比例 4
                 qkv_bias=False,
                 qk_scale=None,
                 drop_ratio=0.,
                 attn_drop_ratio=0.,  # attention 中的 dropout 概率
                 drop_path_ratio=0.,  # attention 中的 droppath 概率
                 act_layer=nn.GELU,
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        Args:
            ...
            in_c (int): 输入通道
            num_classes (int): 分类数
            embed_dim (int): patch embedding 的维度
    ...
    def forward_features(self, x):
        ...
        最后返回分类 token 并传入 pre_logits: 
            return self.pre_logits(x[:, 0]) = [B,768] 提取cls_token的结果
        '''
    ...
    def forward(self, x):
        """
        self.forward_features(x) :  [B,3,224,224] -> [B,768]
